...
import {
  sendGameInitiate_Waiting,
  sendGameInviteNotifications,
  sendGameInviteTournamentNotifications,
  sendWinnerOfGameTournamentNotifications,
...
export async function Send_data_bacnd_the_winner(
  first_player,
  secong_player,
  winner
) {
  ...
  if (response.ok) {
    ...
    sendWinnerOfGameTournamentNotifications(
      user_id,
      "null",
      `system_Tournament_${tournament_id}:${winner}`
    );
  } else {
  ...
  if (winner === user_id) {
    const matchesResponse = await fetch(
      `${BACKEND_URL}/api/tournaments/${tournament_id}/`,
      {
        headers: {
          Authorization: `Bearer ${getToken()}`,
        },
    ...
    if (matches.length <= 2) {
      let system_winner = localStorage.getItem(
      ...
      let system_final = localStorage.getItem(
        `system_Tournament_status_${tournament_id}`
      );
      let system_final_final = localStorage.getItem(
