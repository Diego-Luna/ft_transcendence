...
export async function Game_js() {
  ...
  const maxPaddleY = canvasElement.height - grid - paddleHeight; // Jonathan - 'maxPaddleY' is assigned a value but never used
  ...
  async function renderGameState(state) {
    ...
    if (state.winner) {
      ...
      const result = response.json; // Diego -  'result' is assigned a value but never used
