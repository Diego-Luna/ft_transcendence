    ...
    if (!startButton.dataset.listenerAttached) {
      startButton.addEventListener("click", async function () {
        ...
        if (check_tournament.participants[0].user.id !== userId) {
          showNotification(
            "Sorry, amigo. Only the creator can start the tournament.",
            "error"
          );
          return;
        }
        ...
        try {
          const response = await fetch(
            `${BACKEND_URL}/api/tournaments/${tournamentId}/update_status/`,
            {
              method: "PUT",
              headers: {
...
export async function fetchParticipantsRSVPs(tournamentId) {
  try {
    const response = await fetch(
      `${BACKEND_URL}/api/participants/status/?tournament_id=${tournamentId}`,
      {
        headers: {
          Authorization: `Bearer ${getToken()}`,
          "Content-Type": "application/json",
      ...
      cancelButton.addEventListener("click", async function () {
        ...
        if (tournament.participants[0].user.id === user_id) {
          ...
          if (activeWebSockets[tournamentId]) {
            ...
            // console.error(
            //   "WebSocket connection not found for tournament",
            //   tournamentId
            // );
          }
          ...
          showNotification(
            "Cancellation failed. Only the creator can cancel the tournament."
          );
        }
      });
...
function check_if_i_am_part_of__tournament(tournament) {
  ...
  const pendingTournament = tournament.participants.some(
    (p) => p.user.id === userId
  );
...
async function initializeTournamentWaitingArea() {
  ...
  const creatorUsername = localStorage.getItem(
    "creatorUsername_" + tournamentId
  );
