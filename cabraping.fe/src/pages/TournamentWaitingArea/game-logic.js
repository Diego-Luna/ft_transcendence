...
export async function sendGameAcceptTournament_Waiting(
  new_userId,
  dest_user_id,
  new_myUserName
) {
  ...
  if (waitingIds.length >= 2) {
    for (let i = 1; i < waitingIds.length; i += 2) {
      ...
      if (
        waitingIds[i] === Number(new_userId) &&
        waitingIds[i - 1] === Number(dest_user_id)
      ) {
        find_me = true;
        break;
      }
...
export async function sendGameAcceptTournament_final_Waiting(
  new_userId,
  dest_user_id,
  new_myUserName
) {
  ...
  const waitingIds = update_waiting_list;
  if (
    waitingIds.includes(Number(new_userId)) &&
    waitingIds.includes(Number(dest_user_id))
  ) {
    find_me = true;
  }
...
async function list_of_Tournament(id) {
  ...
  const pendingTournament = tournaments.find(
    (t) =>
      t.status === "in_progress" && t.participants.some((p) => p.user.id === id)
  );
  ...
  pendingTournament.participants.map((participant) =>
    tem_list.push(participant.user.id)
  );
  ...
  if (list_players_id.length >= 2) {
    for (let i = 0; i < list_players_id.length; i += 2) {
      if (list_players_id[i] === userId) {
        ...
        let status = await sendGameInitiate_Waiting(
          userId,
          list_players_id[i + 1]
        );
        if (status.ok) {
...
export async function handle_Tournament_game_invite(tournament_id) {
  ...
  if (status === true) {
    ...
    sendGameInviteTournamentNotifications(
      ...
      myUserName,
      0,
      `system_Tournament_${tournament_id}`
    );
