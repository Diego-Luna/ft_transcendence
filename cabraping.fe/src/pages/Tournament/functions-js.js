...
import { getUserIdFromJWT } from "../Chat/functions-js.js";
...
export async function WS_check_the_tournament_pending() {
...
// Diego, see procesTournament above
    if (pendingTournament) {
...
async function TournamentInit() {
    ...
    WS_check_the_tournament_pending();
...
export async function checkAllParticipantsAccepted(tournamentId) {
    try {
        if (!tournamentId) {
        ...
        if (response.ok) {
            ...
            updategoToWaitingAreaButtonState(participants); // Diego, this function is not defined
            return allAccepted;
        } else {
...
export async function updateParticipantsList(participantNameOrObject, status, isCreator = false, tournamentId) {
