...
import {
  ...
  activeWebSockets,
  handleTournamentWebSocketMessage,
  getUserIdByUsername,
  BACKEND_URL,
  WS_URL,
...
import {
  checkUsers_is_part_of_valid_tournament,
  getTournamentForId,
} from "./cancel.js";
...
export async function WS_check_the_torunament_pending() {
  ...
  const pendingTournament = tournaments.find(
    (t) =>
      t.status === "pending" && t.participants.some((p) => p.user.id === userId)
  );
  const progressTournament = tournaments.find(
    (t) =>
  ...
  if (pendingTournament) {
    ...
    localStorage.setItem(
      `system_Tournament_status_${pendingTournament.id}`,
      "in"
    );
  } else if (progressTournament) {
    localStorage.setItem("currentTournamentId", progressTournament.id);
    localStorage.setItem(
      `system_Tournament_status_${progressTournament.id}`,
      "in"
    );
  } else {
...
export async function Check_if_im_the_creator_to_reload() {
  ...
  const procesTournament = tournaments.find(
    (t) =>
      t.status === "in_progress" &&
      t.participants.some((p) => p.user.id === userId)
  );
...
async function loadTournamentData(tournamentId) {
  try {
    const addParticipantButton = document.getElementById(
      "addParticipantButton"
    );
    addParticipantButton.style.display = "none";

    const response = await fetch(
      `${BACKEND_URL}/api/tournaments/${tournamentId}/`,
      {
        headers: {
          Authorization: `Bearer ${getToken()}`,
          "Content-Type": "application/json",
    ...
    if (response.ok) {
      ...
      const tournamentFormButton = document
      ...
      data.participants.forEach((participant) => {
        ...
        if (participant.status) {
          ...
          if (i === 0) listItem.textContent = `You (Creator)`;
          else {
...
export function connectTournamentWebSocket(tournamentId) {
  if (
    !activeWebSockets[tournamentId] ||
    activeWebSockets[tournamentId].readyState === WebSocket.CLOSED
  ) {
    let jwt = getToken();
...
async function handleAddParticipant(e) {
  ...
  const participantName = document
    .getElementById("participantNameInput")
    .value.trim();
  ...
  if (participantName === currentUser) {
    showNotification(
      "You cannot invite yourself to the party. Have some manners!"
    );
    return;
  }
  ...
  if (!tournamentId) {
    showNotification(
      "No tournament ID found. Please create a tournament first."
    );
    return;
  }
  ...
  if (isOnline === null) {
    ...
    const participantId = await getUserIdByUsername(
      participantName,
      tournamentId
    );
    ...
    if (participantId) {
      ...
      let response_checkuser_repeat =
        await checkUsers_is_part_of_valid_tournament(participantId);

      if (response_checkuser_repeat === null) return;
...
function checkAddParticipantButton(e) {
  ...
  if (addParticipantButton.disabled) {
    ...
    e.preventDefault();
    showNotification(
      "Please create the tournament first before adding participants."
    );
    return;
  }
...
export async function updateParticipantsList(
  participantNameOrObject,
  status,
  isCreator = false
) {
  // console.log("🚨 🚨participantNameOrObject:", participantNameOrObject);
  const participantName =
    typeof participantNameOrObject === "object"
      ? participantNameOrObject.username || participantNameOrObject.name
      : participantNameOrObject;
  const currentUser = localStorage.getItem("username");
  ...
  if (participantsList) {
    ...
    const existingParticipant = Array.from(participantsList.children).find(
      (item) => item.textContent.includes(participantName)
    );
    if (existingParticipant) {
      existingParticipant.textContent = participantName + " - " + status;
      if (!isCreator)
        showNotification("This user has been invited already. Don't be pushy.");
    } else {
      const listItem = document.createElement("li");
      listItem.textContent = isCreator
        ? participantName
        : participantName + " - " + "invited";
      participantsList.appendChild(listItem);
      if (!isCreator)
        showNotification(
          "Invitation successfully sent to " + participantName + ".",
          "success"
        );
    }
    ...
    const goToWaitingAreaButton = document.getElementById(
      "goToWaitingAreaButton"
    );
    if (goToWaitingAreaButton) {
      ...
      goToWaitingAreaButton.parentNode.replaceChild(
        newButton,
        goToWaitingAreaButton
      );
...
async function checkUserOnlineStatus(username) {
  ...
  try {
    ...
    showNotification(
      "An error occurred while checking the user's online status."
    );
    return null;
  }
}
...
async function userExists(username) {
  ...
  try {
    const response = await fetch(
      `${BACKEND_URL}/api/users/${username}/exists/`,
      {
        headers: {
          Authorization: `Bearer ${getToken()}`,
          "Content-Type": "application/json",
    ...
    if (!response.ok) {
      // console.error(
      //   `Error checking user existence: ${response.status} ${response.statusText}`
      // );
      return false;
    }
...
async function handleCreateTournament(e) {
  ...
  const tournamentNameInput_check = document.getElementById(
    "tournamentNameInput"
  );
  ...
  const tournamentName = document
    .getElementById("tournamentNameInput")
    .value.trim();
  if (!tournamentName) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      throw new Error(
        "Failed to create tournament. Server responded with an error."
      );
    }
  } catch (error) {
...
export function acceptTournamentInvitation(tournamentId, username) {
  if (
    !activeWebSockets[tournamentId] ||
    activeWebSockets[tournamentId].readyState !== WebSocket.OPEN
  ) {
    // console.error(
    //   "WebSocket is not connected or already closed for tournament:",
    //   tournamentId
    // );
    ...
    newButton.addEventListener("click", async () => {
      ...
      if (tournament && tournament.id) {
        try {
          ...
          const response = await fetch(
            `${BACKEND_URL}/api/tournaments/${tournament.id}/cancel/`,
            {
              method: "POST",
              headers: {
                Authorization: `Bearer ${getToken()}`,
          ...
          if (response.ok) {
            ...
            if (
              activeWebSockets[tournament.id] &&
              activeWebSockets[tournament.id].readyState === WebSocket.OPEN
            ) {
