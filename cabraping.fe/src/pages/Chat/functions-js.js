...
import { sendAcceptedGameNotifications, sendChannelCreatedNotifications, sendGameInviteNotifications } from "../../components/wcGlobal-functions-send-message.js";
...
export async function Chat_Update_js() {
  ...
  // Diego:
  //59:5  error  'channel' is not defined  no-undef
 // 87:7  error  'channel' is not defined  no-undef
 // 156:7  error  'channel' is not defined  no-undef
...
async function inviteGame(jwt) {
  ...
  async function blockUser(user_id) { // Diego, I changed this to user_id, as you have it below (before it was userId)
...
function handleSaveChannelClick() {
  ...
  fetch(url, requestOptions)
    ...
    .catch((error) => {
      showNotification("There is already a chat.", error);
    });
}
