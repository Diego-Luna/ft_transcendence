...
import {
  startTournament,
  handleTournamentCanceled,
} from "../pages/TournamentWaitingArea/functions-js.js";
...
import {
  ...
  sendDeleteMatchedMessage,
  handleUpdateWaitingList,
  sendUpdateList_of_tournament_Notifications,
} from "./wcGlobal-funcions-send-message.js";
import {
  sendGameAcceptTournament_final_Waiting,
  sendGameAcceptTournament_Waiting,
  system_invite_game_Tournament,
} from "../pages/TournamentWaitingArea/game-logic.js";
  ...
  switch (message.event) {
    ...
    case "game_invite":
      if (message.type === "tournament") {
        ...
        message.message === "system-tournament" ||
        message.message === "system-tournament-final"
      ) {
        return;
      } else {
...
function handleAcceptedInvite(message, tournamentId) {
  ...
  showNotificationPopup(
    message.user_name,
    `Invitation accepted by ${message.user_name}.`
  );
...
function handleRejectedInvite(message, tournamentId) {
  ...
  showNotificationPopup(
    message.user_name,
    `Invitation rejected by ${message.user_name}. Invite someone else or delete the tournament.`
  );
  checkStartTournament(tournamentId);
}

export function sendTournamentInvitation(
  tournamentId,
  participantUsername,
  participantId
) {
  ...
  async function sendMessage() {
    ...
    const tournamentName = localStorage.getItem(
      `tournamentName_${tournamentId}`
    );
    ...
    if (!userId || !recipientId) {
      //   console.error(
      //     "User ID or recipient ID is not set. User ID:",
      //     userId,
      //     "Recipient ID:",
      //     recipientId
      //   );
...
async function updateInviteStatus(tournamentId, accepted) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      showNotificationPopup(
        "Error.",
        "Error updating invite status: " + errorText
      );
      return false;
    }
...
export async function fetchParticipants(tournamentId) {
  try {
    ...
    if (!response.ok) {
      // console.error(
      //   "Error fetching tournament participants:",
      //   response.statusText
      // );
      return null;
    }
...
export function handleTournamentInvite(data, tournamentId) {
  ...
  document.getElementById("acceptTournamentInvite").onclick = async () => {
    ...
    sendUpdateList_of_tournament_Notifications(
      ...
      username,
      0,
      `system_Tournament_${tournamentId}_updatelist`
    );
...
function handleGameInvite(data) {
  ...
  showNotificationPopup(
    data.user_name,
    `You have been matched to play a game! ${data.user_name}`
  );
  //updateParticipantsList(data.user_name, 'invited');
}

export function create_data_for_TournamentWebSocket({
  tournamentId,
  event,
  type,
  dest_user_id,
    ...
    switch (message.event) {
      case "update_user_list":
        localStorage.setItem(
          "id_active_users",
          JSON.stringify(message.user_ids)
        );
        Chat_Update_js();
        Friends_js();
        Users_js();
      ...
      case "update_waiting_list":
        ...
        localStorage.setItem(
          "update_waiting_list",
          JSON.stringify(message.waiting_ids)
        );
...
async function Torunament_game_diego(message) {
  ...
  if (message.dest_user_id === "0" || message.dest_user_id === 0) {
    ...
    if (
      words.length === 2 &&
      `system_Tournament_${tournament_id}` === words[0]
    ) {
      localStorage.setItem(
        `system_Tournament_${tournament_id}_winner`,
        words[1]
      );
      localStorage.setItem(
        `system_Tournament_status_${tournament_id}_final`,
        "final"
      );
    }
  }
...
async function sendGameAccept_Waiting(userId, dest_user_id, myUser) {
  ...
  if (waitingIds.length >= 2) {
    for (let i = 0; i + 1 < waitingIds.length; i += 2) {
      ...
      if (
        Number(waitingIds[i]) === Number(userId) &&
        Number(waitingIds[i + 1]) === Number(dest_user_id)
      ) {
        find_me = true;
        break;
      }
